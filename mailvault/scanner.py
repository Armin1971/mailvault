...
def decode_header_value(value):
    """Dekodiert einen Mail-Header-Wert (kann encoded sein)."""
    if not value:
        return ""
...
def extract_email_address(from_header):
    """Extrahiert die reine E-Mail-Adresse aus dem From-Header."""
    if not from_header:
        return "", ""
...
def parse_date(date_str):
    """Parst ein Mail-Datum in ein datetime-Objekt."""
    if not date_str:
        return None
    try:
        parsed = email.utils.parsedate_to_datetime(date_str)
        # Naive datetime für SQLite
...
def extract_body(msg):
    """Extrahiert den Body einer Mail als Plaintext."""
    body_text = ""
    body_html = ""
    has_html = False
    ...
    # Wenn nur HTML, zu Plaintext konvertieren
...
def get_or_create_sender(addr, name):
    """Holt oder erstellt einen Sender-Eintrag."""
...
def process_message(msg, folder_name, account_id=None):
    """Verarbeitet eine einzelne Mail und speichert sie in der DB."""
...
def scan_thunderbird_profile(profile_path=None):
    """Scannt alle mbox-Dateien im Thunderbird-Profil."""
    ...
    for profile in profiles:
        ...
        for mail_dir in imap_dirs + local_dirs:
            ...
            # Alle Dateien ohne Extension sind potentielle mbox-Dateien
            for f in os.listdir(mail_dir):
                filepath = os.path.join(mail_dir, f)
                # mbox-Dateien haben keine Extension (INBOX, Sent, Drafts, etc.)
                # .msf sind Index-Dateien, .sbd sind Unterordner
                if os.path.isfile(filepath) and "." not in f:
                    try:
                        ...
                        for msg in mbox:
                            ...
                            # Batch-Commit alle 100 Mails
                            if stats["scanned"] % 100 == 0:
                                db.session.commit()

                        if count > 0:
                            stats["folders"].append(
                                {"name": folder_name, "path": filepath, "count": count}
                            )
                        mbox.close()
                    except Exception as e:
                        stats["errors"] += 1
...
def scan_imap_account(account_id, folders=None, limit=None):
    """Scannt Mails direkt via IMAP."""
        ...
        if not folders:
            ...
            for f in folder_list:
                if isinstance(f, bytes):
                    ...
                    if match:
                        fname = (match.group(1) or match.group(2)).decode(
...
def _update_sender_stats():
    """Aktualisiert Absender-Statistiken nach einem Scan."""
